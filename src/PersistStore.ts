...
export default class PersistStore<T = {}> {
  ...
  constructor(options: Options<T>) {
    console.warn('PersistStore is deprecated. Use persistConfigure in constructor instead of PersistStore');

    if (!options.delay) options.delay = 5000;

    observe(this, ({ object }: { object: Partial<T> }) => {
      const keysIntersection = this.getKeysIntersection(options.properties, object);
      const omitSource = this.getOmitSource(object, keysIntersection);
      const isNeedUpdateSnapshot = this.checkNeedToUpdateSnapshot(keysIntersection, omitSource);

      if (isNeedUpdateSnapshot) {
        options.adapter.writeInStorage(this.constructor.name, omitSource).then(() => {
    ...
    options.adapter.readFromStorage<T>(this.constructor.name).then((source) => {
      if (source) {
        const keysIntersection = this.getKeysIntersection(options.properties, source);
  ...
  private getKeysIntersection = (properties: (keyof T)[], object: Partial<T>) => {
  ...
  private getOmitSource = (source: Partial<T>, keysIntersection: (keyof T)[]) => {
    return keysIntersection.reduce<Partial<T>>((prev, curr) => ({ ...prev, [curr]: source[curr] }), {});
  };

  private checkNeedToUpdateSnapshot = (objectKeys: (keyof T)[], omitObject: Partial<T>) => {
