...
const isSerializableProperty = <T, P extends keyof T>(obj: any): obj is SerializableProperty<T, P> => {
  ...
  return keys.every((key) => {
    ...
    consoleDebug(!!mpsConfig.debugMode, `${key} not found in SerializableProperty`, { key, obj });
    return false
  })
}

export const makeSerializableProperties = <T, P extends keyof T>(properties: PersistenceStorageOptions<T, P>['properties']): SerializableProperty<T, P>[] => {
